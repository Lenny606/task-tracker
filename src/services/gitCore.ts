...
// Collects commits across discovered projects for the given day.
export async function collectCommits(targetDate?: string): Promise<GitCommit[]> {
  try {
    ...
    const dateToFetch = targetDate ? new Date(targetDate) : new Date()
