...
import { GitCommit as GitCommitIcon, Clock, User, Hash } from 'lucide-react'
import { getServerCommits } from '../services/git'

export const Route = createFileRoute('/commits')({
  loader: async () => {
    return await getServerCommits()
  },
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto w-full min-h-screen">
      <div className="mb-10 flex items-center justify-between">
        <div>
          ...
          <p className="mt-4 text-lg text-slate-600 dark:text-slate-400 max-w-2xl font-medium">
            Review the latest changes and updates securely integrated from the project's repository.
          </p>
        </div>
        <div className="hidden lg:flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-800/50 rounded-xl ring-1 ring-slate-200 dark:ring-slate-700">
      ...
      <div className="relative border-l-2 border-slate-200 dark:border-slate-800/60 ml-6 space-y-8 pb-12">
        ...
        {commits.length === 0 && (
          <div className="pl-8 text-slate-500 dark:text-slate-400 font-medium">
